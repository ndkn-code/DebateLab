...
export default async function CourseDetailPage({
  ...
  const hasCourseAccess =
    isAdmin ||
    (course.is_published &&
      (await canAccessCourse(supabase, user.id, courseId)));
  ...
  const modulesWithActivities = await Promise.all(
    (modules ?? []).map(async (mod) => {
      const { data: activities } = await supabase
        .from("activities")
        .select(
          "id, title, activity_type, phase, order_index, duration_minutes",
        )
        .eq("module_id", mod.id)
        .eq("is_archived", false)
        .order("order_index");
  ...
  if (!previewMode) {
    ...
    const allActivityIds = modulesWithActivities.flatMap((m) =>
