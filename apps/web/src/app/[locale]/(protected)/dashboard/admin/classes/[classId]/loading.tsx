export default function AdminClassDetailLoading() {
  return (
    <div className="mx-auto w-full max-w-[1440px] px-4 py-5 sm:px-6 lg:px-8 lg:py-7">
      ...
      <div className="mt-5 h-7 w-72 animate-pulse rounded-control bg-surface-container-high" />
      ...
      <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {Array.from({ length: 4 }).map((_, index) => (
          <div key={index} className="h-24 animate-pulse rounded-control border border-outline-variant bg-surface" />
        ))}
      </div>
