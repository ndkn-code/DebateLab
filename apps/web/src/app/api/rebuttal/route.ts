...
import { difficultyPrompts } from "@/lib/rebuttal/difficulty-prompts";
import {
  buildDeepSeekRebuttalMessages,
  getDeepSeekRebuttalPromptPrefixHash,
  hashRebuttalRequest,
} from "@/lib/rebuttal/rebuttal-messages";
...
} from "@/lib/truong-teen/opponent-case-plan";
import {
  analyzeTruongTeenOpponentOutput,
  ensureTruongTeenStandaloneOffense,
} from "@/lib/truong-teen/opponent-quality";
import type {
  AiDifficulty,
  DebateMemory,
  MotionBrief,
...
export async function POST(req: NextRequest) {
  ...
  let opponentCasePlanMetadata: Record<string, unknown> = {};
...
Highlight 3-5 exact quotes that a student should notice. Use only quote strings that appear verbatim in "rebuttal".`;
    ...
    const rebuttalRequestHash = hashRebuttalRequest({
      ...
      corpusContextHash: createHash("sha256")
        ...
        .digest("hex"),
      opponentCasePlanCacheKey: opponentCasePlan?.cacheKey ?? null,
      opponentCasePlanVersion: opponentCasePlan?.version ?? null,
      truongTeenPromptVersion: useTruongTeenPrompt
        ? TRUONG_TEEN_PROMPT_VERSION
        : null,
    });
          ...
          try {
            ...
            if (opponentOffenseGuardrail) {
              if (opponentOffenseGuardrail.insertedParagraph) {
                send("delta", {
                  text: `\n\n${opponentOffenseGuardrail.insertedParagraph}`,
                });
              }
              structuredResponse = {
                ...structuredResponse,
                rebuttal: opponentOffenseGuardrail.text,
              };
            }
    ...
    getPostHogServer().capture({
      ...
      properties: {
        ...
        opponent_case_plan_source:
          opponentCasePlanMetadata.opponentCasePlanSource,
        opponent_case_plan_cache_hit:
