...
export async function GET(request: NextRequest, { params }: { params: Promise<{ materialId: string }> }) {
  ...
  try {
    ...
    if (!placementId || !versionId || !renditionId) return NextResponse.json({ error: "placementId, versionId, and renditionId are required." }, { status: 400 });
    ...
    return NextResponse.json({ ok: false, error: error instanceof Error ? error.message : "Could not load material preview." }, { status: 500 });
  }
}
