    ...
    if (isDevAdminBypassEnabled()) {
      if (id)
        return NextResponse.json({
          campaign: devCampaigns.find((item) => item.id === id),
          results: devResults,
        });
      return NextResponse.json({
  ...
  try {
    ...
    if (isDevAdminBypassEnabled()) {
      if (action === "resolve")
        return NextResponse.json({
          count:
            (body.audience as { type?: string })?.type === "admin_test"
              ? 1
              : 12,
        });
      if (action === "save") {
      ...
      if (
        ["approve", "schedule", "cancel", "pause", "resume"].includes(
          String(action),
        )
      ) {
        ...
        if (action === "approve") {
          ...
          campaign.audienceSnapshotCount = 1;
        }
