...
import { materialFinalizeSchema, materialIngestSchema, materialRetrySchema } from "@/lib/api/class-lms/material-pipeline/contracts";
import { createMaterialIngest, finalizeMaterialIngest } from "@/lib/api/class-lms/material-pipeline/service";
...
async function actorFor(client: Awaited<ReturnType<typeof createClient>>, clubId: string, scopeClassId: string | null) {
  if (scopeClassId) {
...
export async function prepareLmsMaterialUpload(raw: unknown) {
  ...
  const actorId = await actorFor(client, parsed.clubId, parsed.scopeClassId ?? null);
...
export async function finalizeLmsMaterialUpload(raw: unknown) {
  ...
  const result = await finalizeMaterialIngest(client, parsed.ingestionId, parsed.sha256);
  if (result.status === "queued") await enqueueMaterialProcessing({ materialId: result.material_id, versionId: result.id, idempotencyKey: result.idempotency_key });
  return { materialId: result.material_id, versionId: result.id, status: result.status };
}
...
export async function retryLmsMaterialConversion(raw: unknown) {
  ...
  const queued = await admin.from("lms_material_versions").update({ processing_status: "queued", failure_reason: null, updated_at: new Date().toISOString() }).eq("id", version.id).eq("processing_status", "failed").not("original_path", "is", null).select("*").maybeSingle();
  ...
  await enqueueMaterialProcessing({ materialId: version.material_id, versionId: version.id, idempotencyKey });
  return { materialId: version.material_id, versionId: version.id, status: "queued" as const, replay: false };
}
