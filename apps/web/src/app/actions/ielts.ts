...
/**
 * IELTS content-authoring server actions (WS-1.1). Admin-guarded mutation surface
 * for the authoring UI. Every action: verify admin → call the canonical
 * lib/api/ielts create/update path (typed + Zod) → log → revalidate. Data access
 * lives in lib/api/ielts, never inline here.
 */
