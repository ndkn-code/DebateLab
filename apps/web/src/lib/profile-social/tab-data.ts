...
function assertProfileReadsEnabled() {
...
export async function getProfileAnalyticsTabData(
  ...
  const { supabase } = await getRpcContext();
  assertProfileReadsEnabled();
...
export async function getProfileActivityFeedData(
  ...
  const { supabase, userId } = await getRpcContext();
  assertProfileReadsEnabled();
...
async function getSelfFallbackActivityFeedData(
  ...
  let sessionsQuery = client
    ...
    .select(
      "id, topic_title, topic_category, total_score, duration_seconds, practice_language, created_at",
    )
    .eq("user_id", userId)
  ...
  const sessionItems = (
    (sessionsRes.data ?? []) as Array<Record<string, unknown>>
  ).map((session) => ({
    id: `practice:${String(session.id)}`,
    kind: "practice" as const,
    title: String(session.topic_title ?? "Practice session"),
    subtitle:
      typeof session.topic_category === "string"
    ...
    score:
      ...
      Number.isFinite(session.total_score)
        ? session.total_score
        : null,
    durationMinutes:
  ...
  const activityItems = (
    ...
    .map((activity) => {
      ...
      return {
        ...
        xpEarned:
          typeof activity.xp_earned === "number" &&
          Number.isFinite(activity.xp_earned)
            ? activity.xp_earned
            : 0,
        score: null,
...
async function getSelfFallbackAchievementsData(
  ...
  const [
    ...
    client
      .from("profiles")
      .select(
        "total_sessions_completed, streak_current, streak_longest, streak_last_active_date, total_practice_minutes, level",
      )
      .eq("id", userId)
      .single(),
  ...
  const scores = ((scoresRes.data ?? []) as Array<Record<string, unknown>>).map(
    (row) => (typeof row.total_score === "number" ? row.total_score : 0),
  );
  const maxScore = scores.length > 0 ? Math.max(...scores) : 0;
  const completedCourses = Array.isArray(coursesRes.data)
  ...
  const achievements = (
    (achievementsRes.data ?? []) as Array<Record<string, unknown>>
  ).map((achievement) => {
    const id = String(achievement.id);
    const conditionType = String(achievement.condition_type ?? "");
    const target =
      typeof achievement.condition_value === "number"
    ...
    return {
      ...
      titleReward:
        typeof achievement.title_reward === "string"
      ...
      sortOrder:
        typeof achievement.sort_order === "number" ? achievement.sort_order : 0,
      unlocked,
      unlockedAt,
      progressValue: value,
  ...
  return {
    ...
    categories: Array.from(
      new Set(
        achievementsWithFeatured.map((achievement) => achievement.category),
      ),
    ).sort(),
    unlockedCount: achievementsWithFeatured.filter(
      (achievement) => achievement.unlocked,
    ).length,
