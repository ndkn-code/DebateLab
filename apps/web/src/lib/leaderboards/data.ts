...
export interface GetLeaderboardPageDataOptions {
  ...
  leaderboardLanguage?: LeaderboardLanguage | string | null;
  supabaseClient?: LeaderboardSupabaseLikeClient;
}

const LEDGER_TABLE_CHECKS = [
  { table: "xp_seasons", column: "id" },
  { table: "xp_events", column: "leaderboard_language" },
  { table: "xp_season_user_totals", column: "leaderboard_language" },
  { table: "xp_season_org_totals", column: "leaderboard_language" },
  { table: "leaderboard_user_leagues", column: "leaderboard_language" },
  { table: "leaderboard_season_user_cohorts", column: "leaderboard_language" },
  { table: "leaderboard_season_results", column: "leaderboard_language" },
] as const;
    ...
    if (error) {
      return {
        ...
        reason: isMissingLeaderboardLedgerError(error)
          ...
          : error.code === "PGRST204"
            ? `Leaderboard ledger column '${check.table}.${check.column}' is not available yet.`
...
async function getLedgerLeaderboardPageData(
  ...
  const v2Result = await client.rpc("get_leaderboard_page_data_v2", {
    ...
    p_leaderboard_language: leaderboardLanguage,
  });

  const { data, error } = v2Result;
...
export async function getLeaderboardPageData(
  ...
  const leaderboardLanguage = coerceLeaderboardLanguage(
    options.leaderboardLanguage,
    "en"
  );
  ...
  return getLedgerLeaderboardPageData(supabase, viewerUserId, leaderboardLanguage);
}
