 ...
 */
export const DebateFeedbackEnvelopeSchema = z
  .object({
    content: z.object({
      score: z.number(),
      claimClarity: z.number(),
      evidenceSupport: z.number(),
      logicCoherence: z.number(),
      counterArgument: z.number(),
    }),
    structure: z.object({
      score: z.number(),
      introduction: z.number(),
      bodyOrganization: z.number(),
      conclusion: z.number(),
    }),
    language: z.object({
      score: z.number(),
    ...
    persuasion: z.object({
      score: z.number(),
      audienceAwareness: z.number(),
      impactfulness: z.number(),
    }),
    totalScore: z.number(),
...
export async function generatePracticeFeedback(
  ...
  const maxOutputTokens =
    input.practiceTrack !== "speaking" && input.isFullRound ? 12_000 : 6_144;
  ...
  let feedback = normalizeDebateScore(
    result.output as unknown as DebateScore,
    input,
  );
  ...
  const isFullDebateRound =
    input.practiceTrack !== "speaking" && input.isFullRound;
