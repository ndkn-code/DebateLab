import type { SandboxConversionRequest, SandboxConversionResult } from "./contracts";
...
function sandboxConfig() {
  ...
  try {
    ...
    throw new SandboxConfigurationError("VERCEL_SANDBOX_API_URL must be an absolute URL.");
  }
  if (url.protocol !== "https:" && process.env.NODE_ENV === "production") {
    throw new SandboxConfigurationError("Production Sandbox endpoint must use HTTPS.");
  }
 ...
 */
export function createVercelSandboxAdapter(fetchImpl: typeof fetch = fetch): MaterialSandboxAdapter {
  return {
    async convert(request) {
      const config = sandboxConfig();
      const response = await fetchImpl(`${config.endpoint}/v1/material-conversions`, {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        throw new SandboxConversionError(`Sandbox conversion failed (${response.status}).`);
      }
      const body: unknown = await response.json();
      if (!body || typeof body !== "object" || typeof (body as { text?: unknown }).text !== "string") {
        throw new SandboxConversionError("Sandbox returned an invalid conversion result.");
      }
      const result = body as SandboxConversionResult;
      if (result.text.length > 10_000_000) throw new SandboxConversionError("Sandbox result exceeded the conversion limit.");
      return result;
    },
  };
}

export function createFakeSandboxAdapter(result: SandboxConversionResult = { text: "deterministic preview" }): MaterialSandboxAdapter {
