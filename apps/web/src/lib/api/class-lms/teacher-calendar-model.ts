...
export type TeacherCalendarStatus = "scheduled" | "completed" | "cancelled" | "archived";
export type TeacherCalendarScheduleStatus = "active" | "cancelled" | "archived";

export const TEACHER_CALENDAR_VIEWS: readonly TeacherCalendarView[] = ["day", "week", "month", "agenda"];
...
export const TEACHER_CLASS_COLOR_TOKENS = [
  "blue", "teal", "amber", "coral", "violet", "pink", "slate",
] as const;
export type TeacherClassColorToken = (typeof TEACHER_CLASS_COLOR_TOKENS)[number];
...
export interface TeacherCalendarRangeResult {
  ...
  classes: Array<{ id: string; title: string; programType: AdminClassProgram; colorToken: TeacherClassColorToken }>;
}

export interface TeacherCalendarEventDetail extends TeacherCalendarEvent {
  rosterCount: number;
  lessonNotes: string | null;
  materials: Array<{ id: string; title: string; kind: string; required: boolean }>;
  assignments: Array<{ id: string; title: string; dueAt: string | null; relationType: string; submissionCount: number; reviewCount: number }>;
  attendance: { sessionId: string | null; present: number; late: number; absent: number; recorded: number };
  announcements: Array<{ id: string; title: string; body: string; status: string; publishAt: string | null }>;
...
export function isTeacherCalendarView(value: unknown): value is TeacherCalendarView {
  return typeof value === "string" && (TEACHER_CALENDAR_VIEWS as readonly string[]).includes(value);
}

export function isTeacherCalendarColorToken(value: unknown): value is TeacherClassColorToken {
  return typeof value === "string" && (TEACHER_CLASS_COLOR_TOKENS as readonly string[]).includes(value);
}

export function normalizeTeacherCalendarView(value: unknown): TeacherCalendarView {
...
export function normalizeTeacherCalendarPreferences(
  ...
  return {
    ...
    workingHours: { start: normalizeClock(start, "08:00"), end: normalizeClock(end, "20:00") },
    timezoneMode: input?.timezoneMode === "user" || input?.timezoneMode === "fixed" ? input.timezoneMode : "class",
    timezone: validTimezone(input?.timezone) ? input!.timezone! : fallbackTimezone,
  };
}

export function normalizeTeacherCalendarRange(input: TeacherCalendarRangeInput, now = new Date()): TeacherCalendarRange {
  const view = normalizeTeacherCalendarView(input.view);
  const timezone = validTimezone(input.timezone) ? input.timezone! : TEACHER_CALENDAR_DEFAULT_TIMEZONE;
  const anchor = input.anchorDate ?? input.startDate ?? dateKeyInTimezone(now, timezone);
  ...
  if (!startDate || !endDate) {
    if (view === "day") startDate = endDate = anchor;
      ...
      while (endDate.slice(0, 7) !== startDate.slice(0, 7)) endDate = addIsoDateDays(endDate, -1);
    }
  }
  assertIsoDate(startDate);
  assertIsoDate(endDate);
  if (startDate > endDate) throw new Error("Calendar range start must not be after end");
  const days = daysBetween(startDate, endDate) + 1;
  if (days > TEACHER_CALENDAR_MAX_DAYS) throw new Error(`Calendar range must be at most ${TEACHER_CALENDAR_MAX_DAYS} days`);
...
export function expandTeacherScheduleOccurrences(
  ...
  rangeStart: string,
  rangeEnd: string,
): Array<{ scheduleId: string; date: string; startsAt: string; endsAt: string }> {
  const dates = expandScheduleOccurrences(schedule, rangeStart, rangeEnd, TEACHER_CALENDAR_MAX_DAYS + 1)
    .filter((item) => item.date >= rangeStart && item.date <= rangeEnd)
    .filter((item) => !schedule.endDate || item.date <= schedule.endDate);
  const timezone = validTimezone(schedule.timezone) ? schedule.timezone! : TEACHER_CALENDAR_DEFAULT_TIMEZONE;
...
export function sortTeacherCalendarEvents(events: TeacherCalendarEvent[]): TeacherCalendarEvent[] {
  return [...events].sort((a, b) =>
    a.startsAt.localeCompare(b.startsAt) || a.endsAt.localeCompare(b.endsAt) || a.classTitle.localeCompare(b.classTitle) || a.id.localeCompare(b.id),
  );
}
...
export function zonedWallClockToUtc(date: string, time: string, timezone: string): string {
  ...
  if (![hour, minute, Number(second)].every(Number.isFinite) || hour > 23 || minute > 59 || Number(second) > 59) throw new Error("Invalid schedule time");
  const guess = new Date(`${date}T${pad(hour)}:${pad(minute)}:${pad(Number(second))}.000Z`);
  ...
  const renderedUtc = Date.UTC(values.year, values.month - 1, values.day, values.hour, values.minute, values.second);
  const desiredUtc = Date.UTC(Number(date.slice(0, 4)), Number(date.slice(5, 7)) - 1, Number(date.slice(8, 10)), hour, minute, Number(second));
  const result = new Date(guess.getTime() + desiredUtc - renderedUtc);
  const roundTrip = localWallClockParts(result, timezone);
  if (roundTrip.date !== date || roundTrip.hour !== hour || roundTrip.minute !== minute || roundTrip.second !== Number(second)) {
    throw new Error(`Schedule time does not exist in ${timezone} because of a timezone transition`);
  }
  return result.toISOString();
}

function localWallClockParts(value: Date, timezone: string) {
  const parts = new Intl.DateTimeFormat("en-US", { timeZone: timezone, hourCycle: "h23", year: "numeric", month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit", second: "2-digit" }).formatToParts(value);
  const get = (type: string) => Number(parts.find((part) => part.type === type)?.value ?? Number.NaN);
  return { date: `${pad(get("year"))}-${pad(get("month"))}-${pad(get("day"))}`, hour: get("hour"), minute: get("minute"), second: get("second") };
}
...
function assertIsoDate(value: string): asserts value is string {
  ...
  if (!/^\d{4}-\d{2}-\d{2}$/.test(value) || Number.isNaN(parsed.getTime()) || parsed.toISOString().slice(0, 10) !== value) throw new Error("Invalid calendar date");
}
