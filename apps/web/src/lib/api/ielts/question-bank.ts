...
import {
  ...
  toBankQuestionCard,
  type BankFilters,
  type BankQuestionCard,
} from "@/lib/ielts/question-bank/model";
import {
...
async function loadBankQuestionGroup(
  ...
  const [{ data: group, error }, { data: members, error: membersError }] =
    await Promise.all([
      supabase
        .from("ielts_question_groups")
        .select()
        .eq("test_id", question.test_id)
        .eq("group_key", question.group_key)
        .maybeSingle(),
      supabase
  ...
  if (error || membersError)
    throw new Error(
      `getBankQuestion(group) failed: ${(error ?? membersError)?.message}`,
    );
...
export async function listBankQuestions(
  ...
  if (filters.subskillTag)
    query = query.contains("metadata", {
      subskill_tags: [filters.subskillTag],
    });
  if (filters.search)
    query = query.ilike(
      "prompt",
  ...
  const { data, error, count } = await query.range(
    from,
    from + BANK_PAGE_SIZE - 1,
  );
  if (error) throw new Error(`listBankQuestions failed: ${error.message}`);
  const total = count ?? 0;
  return {
    questions: (data ?? []).map(toBankQuestionCard),
    count: total,
    page: filters.page,
    pageSize: BANK_PAGE_SIZE,
    pageCount: Math.max(1, Math.ceil(total / BANK_PAGE_SIZE)),
  };
}
...
export async function getBankQuestion(
  ...
  const supabase = await verifyAdmin(client);
  const [{ data: question, error }, { data: key, error: keyError }] =
    await Promise.all([
      supabase
        .from("ielts_questions")
        .select("*, ielts_tests(title)")
        .eq("id", id)
        .maybeSingle(),
      supabase
  ...
  if (error || keyError)
    throw new Error(`getBankQuestion failed: ${(error ?? keyError)?.message}`);
  if (!question) return null;
  return {
    question,
    key,
    group: await loadBankQuestionGroup(supabase, question),
  };
}

export async function getBankFacets(
  client?: IeltsDbClient,
): Promise<BankFacets> {
  const supabase = await verifyAdmin(client);
  ...
  if (questionsResult.error || testsResult.error)
    throw new Error(
      `getBankFacets failed: ${(questionsResult.error ?? testsResult.error)?.message}`,
    );
  ...
  return {
    ...
    difficulties: unique(
      rows.flatMap((row) => {
        const value = metadataRecord(row.metadata).difficulty;
