...
const row = (
  over: Partial<LibraryTestRow> & Pick<LibraryTestRow, "id">,
): LibraryTestRow => ({
  title: "Untitled",
  slug: "untitled",
  description: null,
...
assert.deepEqual(testCardSkills({ kind: "skill_set", skill: "listening" }), [
  "listening",
]);
...
// ── filterTestCards ─────────────────────────────────────────────────────────
{
  const cards = [
    ...
    toTestCard(row({ id: "t4", kind: "skill_set", skill: "speaking" })),
  ];

  assert.equal(filterTestCards(cards, "all").length, 4);
  assert.deepEqual(
    filterTestCards(cards, "full_mock").map((card) => card.id),
    ["t1"],
  );
  // A full simulation advertises its three computer-delivered sections.
  assert.deepEqual(
