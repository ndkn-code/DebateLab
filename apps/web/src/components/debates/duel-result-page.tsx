...
export function DuelResultPage({ shareCode }: DuelResultPageProps) {
  const {
    ...
    error,
    isLoading,
  } = useDebateDuelRoom(shareCode, "result");
      ...
      <div className="min-h-full bg-background px-4 py-10">
        <div className="mx-auto max-w-3xl rounded-control border border-outline-variant/15 bg-surface p-6 text-center shadow-none">
          ...
          <p className="mt-3 text-on-surface-variant">
            We couldn&apos;t load the result for this duel. Try again or return
            to the arena.
            <span className="mt-1 block type-caption">
              Support code: DUEL-RESULT-01
            </span>
          </p>
        </div>
      </div>
  ...
  if (!room.judgment) {
    if (
      room.outcomeReason === "forfeit" ||
      room.outcomeReason === "abandoned"
    ) {
...
function DuelForfeitResult({ room }: { room: DebateDuelRoomView }) {
  ...
  const winner =
    room.participants.find(
      (participant) => participant.userId !== forfeitedBy,
    ) ?? null;

  return (
    <div className="min-h-full bg-background px-4 py-10">
      <div className="mx-auto max-w-2xl rounded-[12px] border border-outline-variant/15 bg-surface p-6 text-center shadow-none">
        <div className="mb-6 flex justify-center">
          <DuelIllustration
            name={
              cancelled ? "thinkfy_duel_rematch_v1" : "thinkfy_duel_victory_v1"
            }
        ...
        <h1 className="mt-3 type-heading-lg text-on-surface">
          {cancelled
  ...
  return (
    <div className="min-h-full bg-background">
      <SessionReviewShell
        ...
        overall={
          <section className="rounded-[12px] border border-outline-variant/15 bg-surface p-4 shadow-none sm:p-5">
            ...
            <div className="mt-5 grid gap-3 lg:grid-cols-2">
              {(["proposition", "opposition"] as const).map((side) => {
                ...
                const feedback =
                  judgment.participantFeedback[side] ?? emptyFeedback;
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        <div className="type-eyebrow text-primary">{side}</div>
                        <h2 className="mt-1 type-heading-md text-on-surface">
                          <ParticipantProfileName
