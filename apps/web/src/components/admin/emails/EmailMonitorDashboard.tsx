...
export function EmailMonitorDashboard({ data }: Props) {
  ...
  return (
    <AdminV2Frame>
      <div className="min-w-0 space-y-4 px-4 py-4 sm:px-6 lg:px-8 lg:py-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <h1 className="type-heading-lg text-on-surface">Email Monitor</h1>
            <p className="mt-1 w-full max-w-[calc(100vw-2rem)] text-sm leading-6 text-on-surface-variant sm:max-w-2xl">
              Monitor email delivery, engagement, and system health for Thinkfy
              emails via Resend.
            </p>
          </div>
              ...
              {monitorKpis.map((kpi) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    <div
                      className={`flex size-8 items-center justify-center rounded-[8px] border ${toneClass[kpi.tone]}`}
                    >
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-on-surface-variant">
                      Last 14 days across user lifecycle sends.
                    </p>
                  </div>
                  <TrendingUp className="h-5 w-5 text-primary" />
                </div>
                <div className="mt-4 flex h-44 items-end gap-2 border-b border-outline-variant pb-3">
                  {data.trend.map((point) => (
                    <div
                      key={point.date}
                    ...
                    >
                      <div className="flex h-32 w-full items-end justify-center gap-1">
                        <span
                          ...
                          style={{
                            height: `${Math.max(3, (point.sent / maxTrend) * 100)}%`,
                          }}
                        />
                        <span
                          className="w-2 rounded-t bg-secondary"
                          style={{
                            height: `${Math.max(3, (point.delivered / maxTrend) * 100)}%`,
                          }}
                        />
                        <span
                      ...
                      <tbody className="divide-y divide-outline-variant/30">
                        {data.templatePerformance.map((template) => {
                          const openRate =
                            template.sent > 0
                              ? Math.round(
                                  (template.opened / template.sent) * 100,
                                )
                              : 0;
                          const clickRate =
                            template.sent > 0
                              ? Math.round(
                                  (template.clicked / template.sent) * 100,
                                )
                              : 0;
                          const deliveredRate =
                            template.sent > 0
                            ...
                            <tr key={template.templateKey}>
                              <td className="px-5 py-4">
                                <p className="font-semibold text-on-surface">
                                  {template.templateKey.replaceAll("_", " ")}
                                </p>
                              ...
                              <td className="px-4 py-4">
                                <div className="flex items-center gap-2">
                                  <span className="w-10 text-xs font-semibold text-on-surface">
                                    {openRate}%
                                  </span>
                    ...
                    <tbody className="divide-y divide-outline-variant/30">
                      {data.recentMessages.length === 0 ? (
                        ...
                        data.recentMessages.map((message) => (
                          <tr key={message.id}>
                            <td className="max-w-[220px] truncate px-5 py-4 text-on-surface">
                              {message.toEmail}
                            </td>
                            <td className="px-5 py-4">
                              <p className="font-semibold text-on-surface">
                                {message.templateKey.replaceAll("_", " ")}
                              </p>
                              <p className="text-xs text-muted-foreground">
                                {message.locale} · {message.category}
                              </p>
                            ...
                            <td className="px-5 py-4">
                              <span
                                className={`rounded-full px-2.5 py-1 text-xs font-bold ${statusClass[message.status] ?? statusClass.queued}`}
                              >
                                {message.status}
                              </span>
                ...
                <div className="rounded-3xl border border-outline-variant/40 bg-surface-container-lowest p-5 shadow-token-card">
                  ...
                  <div className="mt-4 space-y-3">
                    {data.webhookEvents.length === 0 ? (
                      <p className="text-sm text-on-surface-variant">
                        No webhook errors or events yet.
                      </p>
                    ) : (
                      data.webhookEvents.slice(0, 5).map((event) => (
                        ...
                        >
                          ...
                          {event.errorMessage ? (
                            <p className="mt-1 text-xs text-error-dim">
                              {event.errorMessage}
                            </p>
                          ) : null}
                        </div>
                ...
                <div className="rounded-3xl border border-outline-variant/40 bg-surface-container-lowest p-5 shadow-token-card">
                  ...
                  <div className="mt-4 space-y-3">
                    {data.cronRuns.length === 0 ? (
                      <p className="text-sm text-on-surface-variant">
                        Email dispatch has not run yet.
                      </p>
                    ) : (
                      data.cronRuns.slice(0, 5).map((run) => (
                        ...
                        >
                          <div className="flex items-center justify-between">
                            <span
                              className={`rounded-full px-2.5 py-1 text-xs font-bold ${statusClass[run.status] ?? statusClass.queued}`}
                            >
                              {run.status}
                            </span>
                            <span className="text-xs text-muted-foreground">
                          ...
                          <p className="mt-2 text-xs text-on-surface-variant">
                            {run.sentCount} sent · {run.skippedCount} skipped ·{" "}
                    ...
                    {data.suppressions.length === 0 ? (
                      ...
                      data.suppressions.slice(0, 5).map((suppression) => (
                        ...
                        >
                          ...
                          <p className="text-xs text-muted-foreground">
                            {suppression.reason} ·{" "}
                            {suppression.category ?? "all streams"} ·{" "}
                            {suppression.source}
                          </p>
                        </div>
                      ))
                ...
                <div className="rounded-3xl border border-outline-variant/40 bg-surface-container-lowest p-5 shadow-token-card">
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-3 text-xs">
                    ...
                    <div className="rounded-2xl bg-surface-container-low p-3">
                      ...
                      <p className="mt-1 font-semibold text-on-surface">
                        {data.domain.openTracking === null
                          ? "Unknown"
                          : data.domain.openTracking
                            ? "Enabled"
                            : "Disabled"}
                      </p>
                    </div>
                    <div className="rounded-2xl bg-surface-container-low p-3">
                      ...
                      <p className="mt-1 font-semibold text-on-surface">
                        {data.domain.clickTracking === null
                          ? "Unknown"
                          : data.domain.clickTracking
                            ? "Enabled"
                            : "Disabled"}
                      </p>
                    </div>
