...
const inputClass =
  "h-11 w-full rounded-xl border border-input bg-surface-container-lowest px-3 text-sm text-on-surface outline-none focus:border-ring focus:ring-3 focus:ring-ring/40";

function getCopy(campaign: Campaign, templates: TemplatePayload | null) {
  const defaults =
    templates?.templates.find(
      (item) => item.templateKey === campaign.templateKey,
    )?.locales[campaign.locale].effectiveCopy ?? EMPTY_COPY;
  return {
    ...defaults,
    ...campaign.body,
    subject: campaign.subject || defaults.subject,
  } as TemplateCopy;
}
...
export function EmailCampaignsDashboard() {
  ...
  const [audience, setAudience] = useState<EmailAudienceSegment>({
    type: "admin_test",
  });
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!campaignResponse.ok)
        throw new Error(campaignData.error || t("errors.load"));
      if (!templateResponse.ok)
        throw new Error(templateData.error || t("errors.load"));
      setCampaigns(campaignData.campaigns);
      setOptions(campaignData.options);
      setTemplates(templateData);
      if (!selectedId) {
        const defaults = templateData.templates.find(
          (item: { templateKey: string }) => item.templateKey === "welcome",
        )?.locales.en.effectiveCopy;
  ...
  function selectCampaign(campaign: Campaign) {
    ...
    setNotice(null);
    setError(null);
    void fetch(`/api/admin/emails/campaigns?id=${campaign.id}`, {
      cache: "no-store",
    })
  ...
  function createNew() {
    const defaults =
      templates?.templates.find((item) => item.templateKey === "welcome")
        ?.locales.en.effectiveCopy ?? EMPTY_COPY;
    setSelectedId(null);
    setName("");
  ...
  function applyTemplate(nextKey: EmailTemplateKey, nextLocale = locale) {
    ...
    const defaults = templates?.templates.find(
      (item) => item.templateKey === nextKey,
    )?.locales[nextLocale].effectiveCopy;
    if (defaults) setCopy(defaults);
  }
  ...
  async function saveDraft() {
    try {
      ...
      const data = await post({
        action: "save",
        campaign: {
          id: selected?.status === "draft" ? selected.id : undefined,
          name,
          templateKey,
          locale,
          audience,
    ...
    <div className="grid min-w-0 grid-cols-1 gap-5 xl:grid-cols-[280px_minmax(0,1fr)]">
              ...
              <button
                ...
                className={cn(
                  ...
                  campaign.id === selectedId
                    ? "border-primary/40 bg-primary-container/50"
      ...
      >
        <section className="rounded-2xl border border-outline-variant/40 bg-surface-container-lowest p-4 shadow-token-card sm:p-6">
          ...
          <div className="mt-5 grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <label>
              ...
              <Select
                value={templateKey}
                onChange={(event) =>
                  applyTemplate(event.target.value as EmailTemplateKey)
                }
            ...
            <label>
              ...
              <Select
                value={locale}
                onChange={(event) => {
                  const next = event.target.value as EmailLocale;
                  setLocale(next);
                  applyTemplate(templateKey, next);
                }}
                disabled={Boolean(selected && selected.status !== "draft")}
              >
          ...
          <div className="mt-5">
            <CampaignEmailTemplateEditor value={copy} onChange={setCopy} />
          </div>
          ...
          <div className="mt-6 rounded-xl border border-outline-variant/40 bg-surface-container-low p-4">
            <div className="flex items-center justify-between gap-3">
              ...
              <span className="inline-flex items-center gap-2 rounded-full bg-surface-container-lowest px-3 py-1 text-sm font-bold text-primary">
            ...
            <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
              <Select
                ...
                onChange={(event) => {
                  ...
                  if (type === "by_plan")
                    setAudience({ type, plan: options.plans[0] ?? "free" });
                  else if (type === "by_locale")
                    setAudience({ type, locale: "en" });
                  else if (type === "by_club")
                    setAudience({
                      type,
                      clubId:
                        options.clubs[0]?.id ??
              ...
              >
                <option value="admin_test">{t("audience.adminTest")}</option>
                <option value="all">{t("audience.all")}</option>
                <option value="by_plan">{t("audience.plan")}</option>
                <option value="by_locale">{t("audience.locale")}</option>
                <option value="by_club">{t("audience.club")}</option>
                <option value="referrers">{t("audience.referrers")}</option>
              </Select>
              {audience.type === "by_plan" ? (
                <Select
                  aria-label={t("audience.plan")}
                  value={audience.plan}
                  onChange={(event) =>
                    setAudience({ type: "by_plan", plan: event.target.value })
                  }
                >
              ...
              {audience.type === "by_locale" ? (
                <Select
                  aria-label={t("audience.locale")}
                  value={audience.locale}
                  onChange={(event) =>
                    setAudience({
                      type: "by_locale",
                      locale: event.target.value as EmailLocale,
                    })
                  }
                >
                  <option value="en">English</option>
              ...
              {audience.type === "by_club" ? (
                <Select
                  aria-label={t("audience.club")}
                  value={audience.clubId}
                  onChange={(event) =>
                    setAudience({ type: "by_club", clubId: event.target.value })
                  }
                >
          ...
          {error ? (
            <div className="mt-4 rounded-xl bg-error-container p-3 text-sm font-semibold text-error-dim">
              {error}
            </div>
          ) : null}
          {notice ? (
            <div className="mt-4 rounded-xl bg-secondary-container p-3 text-sm font-semibold text-secondary-dim">
              {notice}
            </div>
          ) : null}
          ...
          <div className="mt-5 flex flex-wrap gap-3">
            <Button
              ...
              disabled={
                busy || Boolean(selected && selected.status !== "draft")
              }
            >
              {busy ? (
            ...
            <div className="mt-6 grid grid-cols-1 gap-4 border-t border-outline-variant/40 pt-5 lg:grid-cols-2">
              <div className="rounded-xl bg-surface-container-low p-4">
                ...
                <Button
                  ...
                  disabled={
                    busy || !scheduleAt || confirmationName !== selected.name
                  }
              ...
              <div className="rounded-xl border border-error/20 bg-error-container/40 p-4">
                <h3 className="font-bold text-on-surface">{t("send.title")}</h3>
                <input
                  aria-label={t("send.confirm")}
                  className={cn(inputClass, "mt-3")}
                  placeholder={selected.name}
                  value={confirmationName}
                  onChange={(event) => setConfirmationName(event.target.value)}
                ...
                <Button
                  variant="destructive"
                  className="mt-3 w-full"
                  disabled={busy || confirmationName !== selected.name}
        ...
        {selected && selected.status !== "draft" ? (
          <section className="rounded-2xl border border-outline-variant/40 bg-surface-container-lowest p-4 shadow-token-card sm:p-6">
            ...
            <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
              {[
                [t("results.sent"), results.sent],
                [t("results.delivered"), results.delivered],
                [t("results.queued"), results.queued],
                [t("results.delayed"), results.delayed],
                [t("results.bounced"), results.bounced],
                [t("results.complained"), results.complained],
              ].map(([label, value]) => (
                <div
                  key={String(label)}
                ...
                >
                  ...
                  {typeof value === "number" ? (
                    <AnimatedNumber
                      value={value}
                      startOnMount
                      className="mt-2 block text-2xl font-extrabold text-on-surface"
                    />
                  ) : (
                    <span className="mt-2 block type-heading-md font-semibold text-on-surface-variant">
                      —
                      <span className="sr-only">
                        {t("results.notReported")}
                      </span>
                    </span>
                  )}
                </div>
            ...
            <div className="mt-5 h-64">
              <BarChart data={chartData} xDataKey="stage" aspectRatio="2 / 1">
                <Grid horizontal />
                <Bar
                  dataKey="value"
            ...
            <div className="mt-4 flex flex-wrap gap-4 text-sm font-semibold text-on-surface-variant">
              <span>
                {t("results.deliveryRate")}: {results.deliveryRate}%
              </span>
              <span>
                {t("results.openRate")}: {results.openRate}%
              </span>
              <span>
                {t("results.clickRate")}: {results.clickRate}%
              </span>
              <span>
                {t("results.risk")}:{" "}
                {results.bounced +
                  results.failed +
                  results.suppressed +
                  (results.complained ?? 0)}
              </span>
            </div>
            <p className="mt-4 text-xs text-muted-foreground">
              {formatDate(selected.scheduledFor ?? selected.updatedAt)}
            </p>
          </section>
        ) : null}
      </motion.div>
