...
function statusTone(value: AttendanceStatus) {
  ...
  return "border-error/20 bg-error-container text-error-dim";
}
...
function StudentRow({
  ...
  return (
    <div className="grid grid-cols-[1.5fr_0.8fr_0.8fr_auto] items-center gap-3 border-b border-outline-variant/15 px-3 py-3 text-sm last:border-0">
      ...
      <span className="rounded-full bg-success-container px-2 py-1 text-center text-xs font-semibold text-success-dim">
        Active
      </span>
      <div className="flex items-center gap-2">
        <span
          className={cn(
            "w-9 font-semibold",
            (student.attendanceRate30d ?? 0) < 80
              ? "text-warning"
              : "text-success",
          )}
        >
          {student.attendanceRate30d == null
            ? "-"
            : `${student.attendanceRate30d}%`}
        </span>
...
function CourseAssignmentRow({
  ...
  return (
    <div className="flex items-center gap-3 rounded-lg border border-outline-variant/25 bg-background px-3 py-3 transition-all duration-200 hover:-translate-y-0.5 hover:border-primary/20 hover:shadow-sm">
      ...
      <span className="rounded-full bg-success-container px-2 py-0.5 text-xs font-semibold text-success-dim">
...
function AttendanceSheet({
  ...
  const [courseId, setCourseId] = useState(
    data.assignedCourses[0]?.courseId ?? "",
  );
  ...
  const [statuses, setStatuses] = useState<Record<string, AttendanceStatus>>(
    Object.fromEntries(
      data.roster.map((student) => [student.id, "present" as AttendanceStatus]),
    ),
  );
        ...
        <div className="flex-1 space-y-5 overflow-y-auto p-4">
          <div className="space-y-2">
            ...
            <select
              ...
              className="h-11 w-full rounded-lg border border-outline-variant/40 bg-background px-3 text-sm outline-none focus:border-primary"
            >
          ...
          <div className="grid grid-cols-4 gap-2">
            ...
            <div className="rounded-lg border border-outline-variant/30 bg-background p-3 text-center">
              <p className="text-xl font-bold text-error">{counts.absent}</p>
          ...
          <div className="overflow-hidden rounded-lg border border-outline-variant/30">
            ...
            {data.roster.map((student) => (
              <div
                key={student.id}
                className="grid grid-cols-[1fr_172px] items-center gap-2 border-t border-outline-variant/15 px-3 py-2"
              >
          ...
          <label className="block space-y-2">
            ...
            <textarea
              ...
              rows={3}
              className="w-full rounded-lg border border-outline-variant/40 bg-background px-3 py-2 text-sm outline-none focus:border-primary"
            />
          </label>
        </div>
...
export function ClassDetailDashboard({ data }: Props) {
  ...
  const [courseResults, setCourseResults] = useState<
    Array<{ id: string; title: string; category: string | null }>
  >([]);
  ...
  const scheduleData = useMemo<AdminClassSchedulesData>(() => {
    ...
    return {
      ...
      kpis: {
        ...
        activeSchedules: data.schedules.filter(
  ...
  const attendanceTrendData = useMemo(
    () =>
      [...data.attendanceSessions]
        ...
        .slice(-8)
        .map((session) => ({
          date: new Date(`${session.sessionDate}T00:00:00`),
          label: formatDate(session.sessionDate),
          attendance: session.attendanceRate ?? 0,
  ...
  return (
    <PageTransition className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        ...
        <div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="rounded-full bg-success-container px-3 py-1 text-xs font-semibold text-success-dim">
              {data.classInfo.status === "active"
                ? "Active"
                : data.classInfo.status}
            </span>
          </div>
      ...
      {data.loadError && (
        <div className="mt-4 rounded-lg border border-error/20 bg-error-container px-4 py-3 text-sm text-error-dim">
          {data.loadError}
        </div>
      )}
      ...
      <FadeInItem className="mt-6 grid gap-3 rounded-lg border border-outline-variant/30 bg-surface-container-lowest p-4 text-sm shadow-sm transition-all duration-200 hover:border-primary/15 hover:shadow-md sm:grid-cols-2 lg:grid-cols-5">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="text-xs text-on-surface-variant">Program</p>
            <p className="font-semibold text-on-surface">
              {getProgramLabel(data.classInfo.programType)}
            </p>
          </div>
        </div>
        <div className="flex items-start gap-3">
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="text-xs text-on-surface-variant">{t("facts.time")}</p>
            <p className="font-semibold text-on-surface">
              {data.classInfo.meetingSchedule ?? "-"}
            </p>
          </div>
        </div>
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="font-semibold text-on-surface">
              {formatDate(data.classInfo.startDate)} -{" "}
              {formatDate(data.classInfo.endDate)}
            </p>
          </div>
        </div>
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="text-xs text-on-surface-variant">{t("facts.room")}</p>
            <p className="font-semibold text-on-surface">
              {data.classInfo.room ?? "-"}
            </p>
          </div>
        ...
        <div className="mt-5 grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
          <StaggeredContainer className="grid gap-4 sm:grid-cols-2 lg:col-span-2 xl:grid-cols-4">
            <StatCard
              ...
              value={data.classInfo.studentCount}
              delta={data.roster.length}
              deltaSuffix=" active"
            />
            <StatCard
              icon={<CheckCircle2 className="h-5 w-5" />}
              label={t("metrics.attendance")}
              value={data.classInfo.attendanceRate30d ?? 0}
              format={() =>
                data.classInfo.attendanceRate30d == null
                  ? "-"
                  : `${data.classInfo.attendanceRate30d}%`
              }
              spark={attendanceTrendData.map((point) => point.attendance)}
              sparkTone={3}
            />
            <StatCard
              icon={<BookOpen className="h-5 w-5" />}
              label={t("metrics.courses")}
          ...
          <ChartCard
            ...
            eyebrow="Attendance"
            subtitle="Recent session attendance rate for this class."
          >
            {attendanceTrendData.length ? (
              ...
              <ChartEmpty
                ...
                description="Take attendance to populate this trend."
              />
            )}
          </ChartCard>
          <FadeInItem className="rounded-lg border border-outline-variant/30 bg-surface-container-lowest p-4 shadow-sm transition-all duration-200 hover:border-primary/15 hover:shadow-md">
            <div className="mb-3 flex items-center justify-between">
              ...
              <button
                ...
                className="rounded-lg border border-outline-variant/30 px-3 py-1.5 text-xs font-semibold text-primary transition-all duration-200 hover:-translate-y-0.5 hover:bg-surface-container active:scale-[0.98]"
              >
                {t("viewAll")}
              </button>
            </div>
            <div className="overflow-hidden rounded-lg border border-outline-variant/20">
              {data.roster.slice(0, 5).map((student) => (
          ...
          <FadeInItem className="rounded-lg border border-outline-variant/30 bg-surface-container-lowest p-4 shadow-sm transition-all duration-200 hover:border-primary/15 hover:shadow-md">
            <div className="mb-3 flex items-center justify-between">
              <h2 className="font-bold text-on-surface">
                {t("courses.title", { count: data.assignedCourses.length })}
              </h2>
              <button
                onClick={() => setTab("courses")}
                className="rounded-lg bg-primary px-3 py-1.5 text-xs font-semibold text-on-primary transition-all duration-200 hover:-translate-y-0.5 hover:bg-primary/90 active:scale-[0.98]"
              >
                {t("courses.assign")}
              </button>
            </div>
            <div className="space-y-2">
              {data.assignedCourses.slice(0, 5).map((course) => (
                <CourseAssignmentRow
      ...
      {tab === "students" && (
        <FadeInItem className="mt-5 rounded-lg border border-outline-variant/30 bg-surface-container-lowest p-4 shadow-sm transition-all duration-200 hover:border-primary/15 hover:shadow-md">
          ...
          <div className="overflow-hidden rounded-lg border border-outline-variant/20">
            ...
            {data.roster.length === 0 && (
              <p className="px-3 py-12 text-center text-sm text-on-surface-variant">
                {t("roster.empty")}
              </p>
            )}
          </div>
      ...
      {tab === "courses" && (
        <FadeInItem className="mt-5 rounded-lg border border-outline-variant/30 bg-surface-container-lowest p-4 shadow-sm transition-all duration-200 hover:border-primary/15 hover:shadow-md">
            ...
            <div className="w-full md:w-80">
              <SearchPanel
                ...
                onSearch={(query) => {
                  ...
                  startTransition(async () => {
                    const results = await searchCoursesForClass(
                      query,
                      data.classInfo.id,
                    );
                    setCourseResults(
          ...
          <div className="grid gap-2">
            ...
            {data.assignedCourses.length === 0 && (
              <p className="rounded-lg border border-dashed border-outline-variant/30 px-3 py-12 text-center text-sm text-on-surface-variant">
                {t("courses.empty")}
              </p>
            )}
          </div>
        ...
        <div className="mt-5 space-y-4">
          <FadeInItem className="flex flex-col gap-3 rounded-lg border border-outline-variant/30 bg-surface-container-lowest p-4 shadow-sm transition-all duration-200 hover:border-primary/15 hover:shadow-md md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p className="text-sm text-on-surface-variant">
                Display-only meeting patterns for this class. Attendance stays
                manual.
              </p>
            </div>
            <button
          ...
          <StaggeredContainer className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            <StatCard
              ...
              label="Upcoming"
              value={scheduleData.kpis.upcomingMeetings}
            />
            <StatCard
              icon={<Repeat2 className="h-5 w-5" />}
              label="Active rules"
              value={scheduleData.kpis.activeSchedules}
            />
            <StatCard
              icon={<Clock3 className="h-5 w-5" />}
              label="Weekly hours"
              value={scheduleData.kpis.weeklyHours}
      ...
      {tab === "attendance" && (
        <FadeInItem className="mt-5 rounded-lg border border-outline-variant/30 bg-surface-container-lowest p-4 shadow-sm transition-all duration-200 hover:border-primary/15 hover:shadow-md">
          <div className="mb-4 flex items-center justify-between">
            ...
            <button
              onClick={() => setAttendanceOpen(true)}
              className="inline-flex h-10 items-center gap-2 rounded-lg bg-primary px-4 text-sm font-semibold text-on-primary transition-all duration-200 hover:-translate-y-0.5 hover:bg-primary/90 active:scale-[0.98]"
            >
              <CalendarDays className="h-4 w-4" />
              {t("takeAttendance")}
            </button>
          ...
          <div className="mb-5 grid gap-4 xl:grid-cols-2">
            <ChartCard
              ...
              eyebrow="Trend"
              subtitle="Recent session attendance percentage."
            >
              {attendanceTrendData.length ? (
                <div className="h-60">
                  <AreaChart data={attendanceTrendData}>
            ...
            <ChartCard
              title="Session mix"
              eyebrow="Attendance"
              subtitle="Present, late, and absent counts by session."
            >
              {attendanceTrendData.length ? (
                <div className="h-60">
                  <BarChart
                ...
                <ChartEmpty
                  title="No session mix yet"
                  description="Present, late, and absent counts appear after attendance is saved."
                />
              )}
            </ChartCard>
          </div>
      ...
      {attendanceOpen && (
        <AttendanceSheet data={data} onClose={() => setAttendanceOpen(false)} />
      )}
      {scheduleOpen && (
        <ScheduleEditor
          classes={[data.classInfo]}
          ...
          >
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-2">
              <label className="sm:col-span-2">
                ...
                <input
                  name="title"
                  defaultValue={data.classInfo.title}
                  required
              ...
              <ClassProgramFields
                defaultProgram={data.classInfo.programType}
                defaultLevel={data.classInfo.gradeLevel}
              />
              <label>
                <span className="text-xs font-semibold text-on-surface-variant">
                  {t("fields.startDate")}
                ...
                <input
                  type="date"
                  name="startDate"
                  defaultValue={data.classInfo.startDate ?? ""}
                  className="mt-1 h-11 w-full rounded-lg border border-outline-variant/40 bg-background px-3 text-sm outline-none focus:border-primary"
                />
              </label>
              <label>
                ...
                <input
                  type="date"
                  name="endDate"
                  defaultValue={data.classInfo.endDate ?? ""}
                  className="mt-1 h-11 w-full rounded-lg border border-outline-variant/40 bg-background px-3 text-sm outline-none focus:border-primary"
                />
              </label>
              <label>
                ...
                <input
                  ...
                  defaultValue={data.classInfo.meetingSchedule ?? ""}
                  className="mt-1 h-11 w-full rounded-lg border border-outline-variant/40 bg-background px-3 text-sm outline-none focus:border-primary"
                />
              </label>
              <label>
                ...
                <input
                  name="room"
                  defaultValue={data.classInfo.room ?? ""}
                  className="mt-1 h-11 w-full rounded-lg border border-outline-variant/40 bg-background px-3 text-sm outline-none focus:border-primary"
                />
              </label>
              <label className="sm:col-span-2">
                ...
                <textarea
                  name="description"
                  defaultValue={data.classInfo.description ?? ""}
                  rows={3}
                  className="mt-1 w-full rounded-lg border border-outline-variant/40 bg-background px-3 py-2 text-sm outline-none focus:border-primary"
                />
              </label>
              <input
                type="hidden"
                name="status"
                value={data.classInfo.status}
              />
            </div>
            <div className="mt-5 flex justify-between gap-2">
              <button
                ...
                disabled={isPending}
                className="h-10 rounded-lg border border-error/20 bg-error-container px-4 text-sm font-semibold text-error-dim transition-all duration-200 hover:-translate-y-0.5 hover:bg-error-container active:scale-[0.98]"
              >
                {t("archive")}
              </button>
              <div className="flex gap-2">
                ...
                <button
                  disabled={isPending}
                  className="h-10 rounded-lg bg-primary px-4 text-sm font-semibold text-on-primary transition-all duration-200 hover:-translate-y-0.5 hover:bg-primary/90 active:scale-[0.98]"
                >
                  {isPending ? t("saving") : t("save")}
                </button>
              </div>
    ...
    <div className="overflow-hidden rounded-lg border border-outline-variant/20">
      ...
      {data.attendanceSessions
        ...
        .map((session) => (
          <div
            key={session.id}
            className="grid grid-cols-[1fr_1fr_0.5fr_0.5fr_0.5fr_0.7fr] border-t border-outline-variant/15 px-3 py-2 text-sm"
          >
            ...
            <span className="text-success">{session.present}</span>
            <span className="text-warning">{session.late}</span>
            <span className="text-error">{session.absent}</span>
...
function AttendanceGrid({ data }: { data: AdminClassDetailData }) {
  const t = useTranslations("admin.classes.detail.attendance");
  if (
    data.attendanceGrid.sessions.length === 0 ||
    data.attendanceGrid.students.length === 0
  ) {
    return (
      <p className="rounded-lg border border-dashed border-outline-variant/30 px-3 py-10 text-center text-sm text-on-surface-variant">
        {t("empty")}
          ...
          <tr>
            ...
            {data.attendanceGrid.sessions.map((session) => (
              <th key={session.id} className="px-3 py-2 text-center">
                ...
                <div className="max-w-24 truncate type-caption font-normal normal-case">
                  {session.courseTitle}
                </div>
              </th>
            ))}
        ...
        <tbody>
          {data.attendanceGrid.students.map((student) => (
            <tr key={student.id} className="border-t border-outline-variant/15">
              <td className="sticky left-0 z-10 bg-surface-container-lowest px-3 py-2 font-semibold text-on-surface">
                {student.displayName}
              </td>
              {data.attendanceGrid.sessions.map((session) => {
