          ...
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-outline-variant px-4 py-3">
            ...
            <div className="flex flex-wrap gap-2">
              <a
                ...
                className="inline-flex h-9 items-center gap-2 rounded-lg border border-outline-variant bg-surface-container-lowest px-3 text-xs font-bold text-on-surface"
              >
            ...
            {eventMixData.length ? (
              <div className="h-52">
                <BarChart
                  data={eventMixData}
                  xDataKey="type"
                  margin={{ left: 24, right: 20, bottom: 48 }}
                >
                  <Grid horizontal />
                  <Bar
                    dataKey="events"
...
function ClubEventEditor({
  ...
  const [eventType, setEventType] = useState<ClubEventType>(
  ...
  const [startTime, setStartTime] = useState(
    (event?.startTime ?? "17:00:00").slice(0, 5),
  );
  const [endTime, setEndTime] = useState(
    (event?.endTime ?? "18:30:00").slice(0, 5),
  );
  const [frequency, setFrequency] = useState<RecurrenceFrequency>(
    event?.recurrenceRule.frequency ?? "weekly",
  );
  const [interval, setInterval] = useState(
    String(event?.recurrenceRule.interval ?? 1),
  );
  const [weekdays, setWeekdays] = useState<RecurrenceWeekday[]>(
    event?.recurrenceRule.weekdays ?? ["MO"],
  );
  const [endMode, setEndMode] = useState<RecurrenceEndMode>(
    event?.recurrenceRule.endMode ?? "on_date",
  );
      ...
      >
            ...
            <label>
              ...
              <select
                ...
                onChange={(changeEvent) =>
                  setEventType(changeEvent.target.value as ClubEventType)
                }
                className="mt-1 h-11 w-full rounded-lg border border-outline-variant bg-background px-3 text-sm outline-none focus:border-primary"
              >
          ...
          <section className="rounded-lg border border-outline-variant bg-background p-3">
            ...
            <div className="mt-3 grid gap-3 sm:grid-cols-2">
              <select
                value={frequency}
                onChange={(changeEvent) =>
                  setFrequency(changeEvent.target.value as RecurrenceFrequency)
                }
              ...
              <input
                ...
                value={interval}
                onChange={(changeEvent) =>
                  setInterval(changeEvent.target.value)
                }
                className="h-10 rounded-lg border border-outline-variant bg-surface-container-lowest px-3 text-sm outline-none focus:border-primary"
              />
            </div>
            ...
            {frequency !== "none" && (
              <div className="mt-3 grid gap-3 sm:grid-cols-2">
                <select
                  value={endMode}
                  onChange={(changeEvent) =>
                    setEndMode(changeEvent.target.value as RecurrenceEndMode)
                  }
                  className="h-10 rounded-lg border border-outline-variant bg-surface-container-lowest px-3 text-sm outline-none focus:border-primary"
                >
                ...
                {endMode === "on_date" ? (
                  <input
                    ...
                    value={until}
                    onChange={(changeEvent) =>
                      setUntil(changeEvent.target.value)
                    }
                    className="h-10 rounded-lg border border-outline-variant bg-surface-container-lowest px-3 text-sm outline-none focus:border-primary"
                  />
                ) : (
            ...
            <p className="mt-3 rounded-lg border border-outline-variant bg-surface-container-lowest px-3 py-2 text-xs font-semibold text-on-surface-variant">
              {recurrenceSummary}
            </p>
          </section>
        </div>
        ...
        <div className="flex flex-col-reverse gap-2 border-t border-outline-variant p-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <button
            type="submit"
            disabled={isPending}
            className="inline-flex h-11 items-center justify-center gap-2 rounded-lg bg-primary px-4 text-sm font-bold text-on-primary disabled:bg-surface-container-high"
          >
            {isPending ? (
