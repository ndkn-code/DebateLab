...
select is((select count(*)::integer from public.lms_occurrence_roster_snapshots), 2, 'Active student sees own snapshots only when the parent occurrence is visible');
...
select is((select count(*)::integer from public.lms_occurrence_roster_snapshots), 1, 'Removed student sees own snapshots only for visible historical occurrences');
