...
begin
  ...
  if new.section_id is not null then
    ...
    if v_section.skill <> (select skill from public.ielts_questions where id = new.question_id) then
  ...
  if v_attempt.blueprint_frozen_at is not null and not exists (
...
begin
  ...
  if v_attempt.blueprint_frozen_at is not null and not exists (
    select 1 from public.ielts_attempt_question_blueprints b
     where b.attempt_id = p_attempt_id and b.section_id = p_section_id and b.question_id = p_question_id
  ) then raise exception 'QUESTION_NOT_IN_FROZEN_ATTEMPT'; end if;
  insert into public.ielts_question_responses (attempt_id, user_id, question_id, section_id, response)
  values (p_attempt_id, v_attempt.user_id, p_question_id, p_section_id, coalesce(p_response, '{}'::jsonb))
  on conflict (attempt_id, question_id) do update
    set section_id = excluded.section_id, response = excluded.response, updated_at = now()
...
begin
  ...
  if exists (select 1 from public.ielts_attempt_sections where attempt_id = p_attempt_id and submitted_at is null) then raise exception 'ATTEMPT_INCOMPLETE'; end if;
