  ...
  payload: {
    ...
    variables: {
      ...
      oneClickUnsubscribeUrl:
        "https://thinkfy.net/api/email/unsubscribe?token=signed",
    },
  },
};
const job = {
  id: "job-1",
  idempotency_key: "notification:job-1",
  channel: "email",
};
...
test("renders an accessible, localized email and preserves plain text", () => {
  ...
  assert.equal(
    content.html.match(/<table\b/g)?.length,
    content.html.match(/<table role="presentation"/g)?.length,
  );
...
test("renders Vietnamese copy with explicit language", () => {
  const content = buildEmailContent(
    event,
    {
      ...recipient,
      display_name: "Minh",
      preferences: { preferred_locale: "vi" },
    },
    job,
    "https://thinkfy.net",
  );
